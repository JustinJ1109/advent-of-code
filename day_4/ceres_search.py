...
GRID = [l.strip() for l in read_input(os.path.join(".", "input.txt"))]
        ...
        for direction in DIRECTIONS:
            ...
            for i, char in enumerate(XMAS):
                ...
                if next_r >= len(GRID) or next_r < 0:
                    break
                elif next_c >= len(GRID[r]) or next_c < 0:
                    break
                word += GRID[next_r][next_c]
            if word == XMAS:
                matches += 1
        return matches
...
def main_2():
    ...
    def check_surround(r, c) -> list:
        ...
        for combo in combos:
            if GRID[r+combo[0][0]][c+combo[1][1]]
        return matches
